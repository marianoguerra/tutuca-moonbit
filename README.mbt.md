...
| **Expression language** | `tscript/` (+ `check/`, `interp/`, `emit_mbt/`, `conformance/`) | Reading the surface tutuca writes: the slot expressions in a view (`.field`, `$method`, `@value`) and the block language of `<script type="tutuca/script">`. Tokenizer, parser, checker, an interpreter for the card runtime, and a MoonBit emitter for the ahead-of-time path. Above `core` rather than inside it, for the reason in the cell above. |
...
| **Render-time filters** | `vdom/filter/` (+ `url/`, `handler/`, `markup/`, `markdown/`), `markdown/` | The half a static pass cannot decide: an attribute VALUE is only known once state has produced it. URL schemes, `on*` handlers, sanitized raw markup, and Markdown rendered straight into vdom nodes. `markdown/` is a CommonMark+GFM parser vendored from mizchi/markdown.mbt — see `markdown/UPSTREAM.md`. |
...
| **Tooling** | `lint/`, `inspector/`, `statedef/`, `viewfile/`, `viewgen/`, `cli/` | The linter (parse-issue rules + a WHATWG-tokenizer structural HTML linter), a schema inspector, the state schema language, the view-file splitter, the ahead-of-time view compiler, and the native `tutuca` CLI. |
...
| **Demos & docs** | `demo/`, `playground/`, `storybook/`, `tutucard/`, `guests/` | 52 ported examples (`storybook/examples/`), browser/wasm demo hosts, an in-browser playground, the compiler-free card playground, a compiled storybook gallery, and nine sample `tutuca:component` guests (eight MoonBit, one Rust — the list is `guests/guests.mjs`). |
