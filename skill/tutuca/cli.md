...
| what does it look like? | a gallery of your own components: `tutuca new-storybook`, then `node build.mjs && python3 -m http.server --directory dist` → [storybook.md](./storybook.md) |
...
`logic:`, and a `pred` or an `invariant` in
`spec:`, are merged into the bucket ahead of your match,
...
a `spec:` section gets the state half: `CounterState`
...
- **A story registry.** A gallery is a wasm page compiled from the project's
  own modules, scaffolded by `tutuca new-storybook` and built by `node build.mjs`.
  Serve its output with a static HTTP server. The native binary loads no user code, and stories are a
...
python3 -m http.server --directory dist/storybook
```
