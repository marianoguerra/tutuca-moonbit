  ...
  Note:
    div():
      p():
        @(it.title)
      " "
      button(~on_click: toggle_view):
        "flip"
      " "
      comment(): " ~as: picks the view for one  only "
      " "
      each(value, key in it.items):
        render(value)
      " "
      each(value, key in it.items):
        render(value, ~as: "edit")
      " "
      each(value, key in it.items):
        render(value, ~as: it.view)
      " "
      comment(): " view chosen by a field at runtime "
      " "
      comment(): " ~push_view forces a view on every component rendered under the host "
      " "
