// Browser loader for the dynamic-wasm-component demo.
//
// Extends the counter_wasm loader (same `jscore` + `tdom` namespaces) with:
//  - listener dedup in tdom.add_listener: the host re-runs install() after a
//    bundle loads to cover events only guest views use, so repeats must be
//    no-ops (one delegated listener per event name per node).
//  - a `tcomp` namespace bridging the wasm-gc host to jco-transpiled
//    `tutuca:component` guests: bundle/instance tables, a JS-side arena
//    backing the guests' `values` imports, a control-message buffer drained
//    per dispatch, and JSON codecs at the host boundary (see
//    dyncomp/host/wasm/glue.mbt for the conventions).
...
export async function loadWasm(wasmUrl) {
  ...
  const imports = {
    ...
    tcomp: createTcompImports(() => exports),
