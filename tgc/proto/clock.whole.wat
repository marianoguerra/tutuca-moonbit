(module
  ...
  ;; The ONE recursion group, and the only reason it is one: these four refer to
  ;; each other and nothing else here does. `tgc/1` put all twenty types in a
  ;; single group, so adding an arm re-typed every module ever built; `tgc/2`
  ;; groups by what actually cycles, and this file has to spell the same
  ;; grouping because a group's identity is the whole group. The MEMBERSHIP and
  ;; the ORDER are the contract; the names are not.
  (rec
  ...
  (global (export "tgc.abi") i32 (i32.const 2))
