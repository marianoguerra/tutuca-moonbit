...
// Output: dist/todo.component.wasm + dist/js/ (ESM + core wasm).
//
...
run('wasm-tools', [
  ...
  '--encoding', 'utf16',
  '-o', join(dist, 'todo.embedded.wasm'),
]);
run('wasm-tools', [
  'component', 'new',
  join(dist, 'todo.embedded.wasm'),
  '-o', join(dist, 'todo.component.wasm'),
]);
run(process.execPath, [
  join(repoRoot, 'node_modules', '@bytecodealliance', 'jco', 'src', 'jco.js'),
  'transpile', join(dist, 'todo.component.wasm'),
  '--instantiation', 'async',
  '-o', join(dist, 'js'),
]);
console.log('built', join(dist, 'todo.component.wasm'), 'and', join(dist, 'js'));
